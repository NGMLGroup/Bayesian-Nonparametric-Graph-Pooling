...
class MolhivModel(torch.nn.Module):
    """
    A variant of the classification model for the ogbg-molhiv dataset.
    ...
    Args:
        ...
        dropout (float): Dropout after each GIN layer and in the readout 
    ...
    def __init__(self, 
        ...
        for _ in range(num_layers_pre):
            ...
            # MP layers
            mlp = MLP([in_channels, hidden_channels, hidden_channels], act=activation, norm=None)
            if self.using_gine:
        ...
        self.mlp = MLP([hidden_channels, hidden_channels, hidden_channels//2, out_channels], 
                        act=activation, norm=None) #, dropout=dropout
